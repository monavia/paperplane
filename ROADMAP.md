...
**Mulai:** 2026-07-20 | **Update prioritas:** 2026-07-25 — Patch v2.2.2 released → Redis Foundation (0.7.0) ✅ → lanjut 0.7.1–0.7.7 swap cache ke Redis. | **Status:** Fase 0 ✅, Patch v2.1.6 ✅, v2.2.0 ✅, v2.2.1 ✅, **v2.2.2 ✅ (released)**, Fase 0.5 ✅, Fase 0.5.5 ✅, Fase 0.6 ✅, Fase 0.7.0 ✅ (Redis connection + CacheAdapter + metrics + rate limiter Redis), Fase 0.7.1 🔴 (swap SearchCache → Redis), Fase 0.7.2–0.7.7 🔴, Fase 0.8 🟡, Fase 0.9 🟡, Fase 1 🟡.
...
## Fase 0.8: Infrastructure 🟡 (1,5 jam — non-blocking)

**Masalah:** Gak ada CI pipeline — PR bisa merge dengan type error. 3 moderate npm vulns unresolved. Secrets rotation gak didokumentasi.

Semua item di sini non-blocking — bisa dikerjain paralel dengan fase lain.

> **Catatan:** Docker & Redis sudah di-setup di Fase 0.7.0.pre. Fase 0.8 sisanya: CI, audit, secrets.

| # | Task | Detail | File | Effort |
|---|------|--------|------|--------|
| 0.8.1 | **CI pipeline** | GitHub Actions: `tsc --noEmit` + `npm test` + `npm run lint` on push + PR. Block merge kalo type error / test fail. | `.github/workflows/ci.yml` (baru) | 1 jam |
| 0.8.2 | **npm audit fix** | `npm audit fix` untuk 3 moderate vulns. Kalo breaking, pin versi dan dokumentasi. | `package-lock.json` | 15 menit |
| 0.8.3 | **Secrets rotation** | Dokumen prosedur ganti Discord token / API keys / Redis password tanpa downtime. `.env.example` sync. | `.env.example` + docs | 30 menit |
...
| # | Task | Detail | File | Effort |
|---|------|--------|------|--------|
| 0.9.1 | **Integration test harness** | Setup test server pake `supertest` (udah di devDeps). Inisialisasi bot minimal + API Express + mock Lavalink. Test health endpoint, queue CRUD, player control. | `src/test/harness.ts` (baru) | 1 hari |
| 0.9.2 | **Error scenario tests** | Test: Redis down (mock), Lavalink timeout, DB disconnect, rate limit exceeded. Verifikasi error message + bot gak crash. | `src/test/errors.test.ts` (baru) | 1 hari |
| 0.9.3 | **Concurrent guild test** | Simulasi 100+ guilds operasi paralel: play, skip, queue add/remove. Deteksi race condition di `withQueueLock`, `advanceQueue`, `trackStart`. | `src/test/concurrent.test.ts` (baru) | 1 hari |
| 0.9.4 | **Benchmark baseline** | Ukur RSS memory, CPU, event loop lag di 0/1/10/100 guilds simulasi. Catat hasilnya di `BENCHMARK.md`. Target 0.6 <1GB di 1000 bisa diverifikasi. | `BENCHMARK.md` (baru) + `src/test/benchmark.ts` | 2 jam |

| 0.9.5 | **Test StateService restore/backup** | Test restoreGuildState (complexity 40), restoreAllStates (complexity 23). Jantung persistence — queue restore, nowPlaying, position. Crash di sini = data loss. | `src/test/state.test.ts` (baru) | 2 jam |
