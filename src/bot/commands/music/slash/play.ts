  ...
  async execute(interaction: any) {
    ...
    try {
      ...
      if (spotifyParsed) {
        ...
        EventBus.emit('metrics:sourcePlayLatency', { guildId: interaction.guildId, ms: startupLatency, source: currentSource });
      ...
      if (!result?.tracks?.length && searchQuery.startsWith("ytsearch:")) {
        ...
        result = await cachedSearch(player, ytmFallback, interaction.user);
      }
