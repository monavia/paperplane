  ...
  async execute(interaction: any) {
    ...
    if (rangeMatch) {
      ...
      if (from < 0 || from >= queue.length || to < 0 || to >= queue.length || from > to) {
        return interaction.editReply({ embeds: [ErrorEmbed.build(`Invalid range. Valid: 0-${queue.length - 1}`)] });
      }
    ...
    if (!isNaN(singleIdx)) {
      if (singleIdx < 0 || singleIdx >= queue.length) {
        return interaction.editReply({ embeds: [ErrorEmbed.build(`Invalid index. Valid range: 0-${queue.length - 1}`)] });
      }
      if (singleIdx === 0) {
        return interaction.editReply({ embeds: [ErrorEmbed.build("Cannot remove the currently playing track.")] });
      }
      const title = queue[singleIdx]?.info?.title || "?";
      const removed = MusicService.removeFromQueue(guildId, singleIdx);
      if (!removed) return interaction.editReply({ embeds: [ErrorEmbed.build("Failed to remove track.")] });
    ...
    if (count === 0) return interaction.editReply({ embeds: [ErrorEmbed.build(`No tracks found matching "${input}".`)] });
    ...
    if (count < 0) {
      ...
      const reply = await interaction.editReply({ embeds: [ErrorEmbed.build(`This will remove ${-count} track(s) matching "${input}". Proceed?`)], components: [row] });
      try {
